                            ...
                            <div className="px-8 pb-8">
                                ...
                                <div className="relative -mt-16 mb-6 flex items-end justify-between">
                                    <div className="flex items-end gap-6">
                                        <div className="relative h-32 w-32 rounded-full border-4 border-white bg-slate-100 shadow-md overflow-hidden">
                                            {user.photo ? (
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                        ...
                                        <div className="pt-4">
                                            ...
                                            <div className="flex flex-wrap gap-2">
                                                {user.skills.map((skill) => (
                                                    <span key={skill} className="rounded-lg bg-slate-100 px-3 py-1.5 text-sm font-medium text-slate-700 border border-slate-200">
                                                        {skill}
                                                    </span>
                                                ))}
                                            </div>
                                    ...
                                    <div className="space-y-6 rounded-2xl bg-amber-50/50 p-6 border border-amber-100">
...
function InfoItem({ icon: Icon, label, value, highlight = false }: { icon: any, label: string, value: string, highlight?: boolean }) {
    return (
        <div className="flex items-center gap-3">
            <div className={cn("flex h-9 w-9 items-center justify-center rounded-lg", highlight ? "bg-amber-100 text-amber-600" : "bg-slate-100 text-slate-500")}>
                <Icon className="h-5 w-5" strokeWidth={2} />
            </div>
            <div>
                <p className="text-xs font-medium text-slate-500">{label}</p>
                <p className={cn("text-sm font-semibold", highlight ? "text-slate-900" : "text-slate-700")}>{value}</p>
            </div>
        </div>
    )
}
