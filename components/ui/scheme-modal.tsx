        ...
        <AnimatePresence>
            {isOpen && (
                <>
                    ...
                    >
                        <div className="relative overflow-hidden rounded-2xl bg-white dark:bg-slate-900 p-6 shadow-2xl border border-slate-100 dark:border-slate-800">
                            {/* Close Button */}
                            <button
                                onClick={onClose}
                                className="absolute right-4 top-4 rounded-full p-2 text-slate-400 dark:text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800 hover:text-slate-600 dark:hover:text-slate-300 transition-colors"
                            >
                                <Xmark className="h-5 w-5" strokeWidth={2.5} />
                            </button>

                            <div className="mt-2">
                                <div className="mb-4 inline-block rounded-full bg-amber-100 dark:bg-amber-900/30 px-3 py-1 text-xs font-bold text-amber-700 dark:text-amber-500">
                                    {content.highlight}
                                </div>

                                <h2 className="text-2xl font-bold text-slate-900 dark:text-white mb-2">{content.title}</h2>
                                <p className="text-slate-600 dark:text-slate-300 mb-6">{content.modal_content.summary}</p>

                                {/* Highlights */}
                                <div className="space-y-3 mb-8">
                                    <h3 className="text-sm font-semibold text-slate-900 dark:text-slate-200 uppercase tracking-wider">{t.key_benefits}</h3>
