...
import { ArrowLeft, Page, Brain } from "iconoir-react/regular"
import { CheckCircle as CheckCircleSolid, HelpCircle as HelpCircleSolid } from "iconoir-react/solid"
        ...
        return (
            <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex flex-col">
                ...
                <div className="flex-1 flex flex-col items-center justify-center p-4">
                    <h1 className="text-2xl font-bold text-slate-800 dark:text-white">Scheme not found</h1>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div className="lg:col-span-2 space-y-8">
                        ...
                        >
                            ...
                            <p className="text-slate-600 dark:text-slate-300 leading-relaxed text-lg">{content.long_description}</p>

                            <div className="mt-8">
                                <h3 className="font-semibold text-slate-900 dark:text-white mb-4">{t.key_highlights}</h3>
                                <div className="grid sm:grid-cols-2 gap-4">
                                    {content.modal_content.highlights.map((item, i) => (
                                        <div key={i} className="flex items-center text-slate-600 dark:text-slate-300 bg-slate-50 dark:bg-slate-800 p-3 rounded-lg">
                            ...
                            <div className="space-y-4">
                                {content.full_page_content.application_steps.map((step, index) => (
                                    <div key={index} className="flex gap-4">
                                        <div className="flex-none flex items-center justify-center w-8 h-8 rounded-full bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-500 font-bold text-sm">
                                            {index + 1}
                                        </div>
                                        <p className="text-slate-600 dark:text-slate-300 text-lg flex-1 pt-0.5">{step.replace(/^\d+\.\s*/, '')}</p>
                                    </div>
                                ))}
                            </div>

                            {content.full_page_content.important_note && (
                                <div className="mt-8 p-4 bg-amber-50 dark:bg-amber-900/10 border border-amber-200 dark:border-amber-900/30 rounded-xl flex gap-3 text-amber-800 dark:text-amber-400">
                    ...
                    <div className="space-y-8">
                        ...
                        >
                            <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-4">{t.documents_required}</h3>
                            <ul className="space-y-3">
                                {content.full_page_content.documents.map((doc, i) => (
                                    <li key={i} className="flex items-start">
                                        ...
                                        <span className="text-slate-600 dark:text-slate-300">{doc}</span>
                                    </li>
                                ))}
                            </ul>
                        </motion.div>
                        ...
                        <motion.div
                            ...
                            className="bg-white dark:bg-slate-900 rounded-2xl p-6 shadow-sm border border-slate-100 dark:border-slate-800"
                        >
                            <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-4">{t.eligibility_criteria}</h3>
                            <ul className="space-y-3">
                                {content.full_page_content.eligibility_detailed.map((item, i) => (
                                    <li key={i} className="flex items-start">
                                        <CheckCircleSolid className="h-5 w-5 text-slate-400 dark:text-slate-500 mr-3 shrink-0" />
